...
namespace ds::amt {
	...
	void ExplicitSequence<BlockType>::clear() {
		BlockType* block = first_;
		while (block != nullptr) {
			BlockType* next = this->accessNext(*block);
			AMS<BlockType>::memoryManager_->releaseMemory(block);
			block = next;
		}
		first_ = nullptr;
		last_ = nullptr;
	...
	bool ExplicitSequence<BlockType>::equals(const AMT& other) {
		...
		auto otherSequence = dynamic_cast<const ExplicitSequence<BlockType>*>(&other);
		if (otherSequence == nullptr) {
			return false;
		}
		BlockType* my = first_;
		BlockType* his = otherSequence->first_;
